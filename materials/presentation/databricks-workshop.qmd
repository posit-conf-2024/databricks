...
:::{.custom2}
- [Download data only if doing something R can do]{style="font-size:80px;"}
- [🔑 Data & processing need to be as **close** as possible]{style="font-size:80px;"}
- [Move most of the processing to Databricks]{style="font-size:80px;"}
- [But how?...]{style="font-size:80px;"}
:::
...
## [Using dplyr & dbplyr]{style="color:#666;"} {background-image="assets/background/slide.png" background-size="1700px" background-color="white"}
...
FROM 'cars'<br/>
]{style="color:#999; font-family:Helvetica Neue; font-weight:300;font-size:35px;"} 
...
:::{.column}
- `print()` - Downloads the top 10 rows by default (Guardrails)
- `collect()` - Downloads **all** of the data
- `pull()` -Downloads **all** of the data, but of a single field/variable. It
will return it to R as a vector.
:::
...
collect(tbl_cars) <br/>
pull(tbl_cars, mpg) 
]{style="color:#666; font-family:Helvetica Neue; font-weight:200;font-size:50px;line-height:1.8;"} 
:::
:::
...
## [Other Advantages]{style="color:#666;"} {background-image="assets/background/slide.png" background-size="1700px" background-color="white"}
...
- Use `dplyr` against over a 12 different databases (MSSQL, Oracle, Teradata, etc.)
:::
:::
::: {.column width="30%"}
